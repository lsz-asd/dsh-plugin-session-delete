...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function DeleteSessionDialog() {
      ...
      const name = target.notFound ? target.title || '未命名会话' : (target.title || '未命名会话')
      ...
      return React.createElement(Modal, {
        ...
        React.createElement('div', { key: 'meta', style: metaStyle },
          ...
          target.sessionId
            ? React.createElement(React.Fragment, null,
                ...
                '序列号：', target.sessionId)
            : null),
        target.running
          ? React.createElement('div', { key: 'warn', style: warnStyle }, '⚠ 会话正在运行')
          : null,
        React.createElement('label', { key: 'ack', style: optStyle },
          ...
          '我已了解后果，确认删除'),
        busy ? React.createElement('div', { key: 'busy', style: statusStyle }, '正在删除…') : null,
    ...
    function DeleteSessionButton(props) {
      ...
      return React.createElement('button', {
        type: 'button',
        title: running ? '删除会话（运行中，删除将停止任务）' : '删除会话',
        'aria-label': '删除会话',
        style: btnStyle,
        onClick: openDialog,
    ...
    function ensureSidebarDeleteItem() {
      ...
      item.innerHTML = '<svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg" style="flex:none"><path d="' + TRASH_PATH + '" fill="currentColor"/></svg><span>删除会话</span>'
